...
describe('Blogs app', () => {
  ...
  describe('When logged in', () => {
    ...
    test('a new blog can be created', async ({ page }) => {
      await createBlog(page, 'vegtarian', 'han kang', 'https://www.goodreads.com/book/show/25489025-the-vegetarian')
      await expect(page.getByText('vegtarian han kang')).toBeVisible()
    })

    test('the blog can be liked', async ({ page }) => {
      await createBlog(page, 'vegtarian', 'han kang', 'https://www.goodreads.com/book/show/25489025-the-vegetarian')
      await createBlog(page, 'The Algorithm Design Manual', 'Steven S. Skiena', 'https://www.goodreads.com/book/show/425208.The_Algorithm_Design_Manual?from_search=true&from_srp=true&qid=LP0WKvrvy5&rank=9')
    ...
    test('only the user who added the blog sees the  blog delete button', async({ request, page }) => {
      await request.post('/api/users', {
        data: {
          username: 'minji',
          password: '0507'
    ...
    test('the blogs are arranged in the order according to the likes, the blog with the most likes first.', async ({ page }) => {
      await page.pause()
      await createBlog(page, 'The Algorithm Design Manual', 'Steven S. Skiena', 'https://www.goodreads.com/book/show/425208.The_Algorithm_Design_Manual?from_search=true&from_srp=true&qid=LP0WKvrvy5&rank=9')
      await createBlog(page, 'vegtarian', 'han kang', 'https://www.goodreads.com/book/show/25489025-the-vegetarian')
